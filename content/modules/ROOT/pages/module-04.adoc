...
. Login to the Minio web console with the user *minio* and password *minio123*.
...
. We'll need to get our OpenShift token before we can configure the event destination in Minio. Click on your username in the top right corner of the OpenShift web console and click *Copy login command*.
+
image::openshift/copy-login-ocp.png[OpenShift token,100%,100%]
+
Copy the token to add to the event destination.
+
