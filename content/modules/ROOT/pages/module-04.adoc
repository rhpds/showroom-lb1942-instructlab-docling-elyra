...
. We'll need to get our OpenShift token before we can configure the event destination in Minio. You can get the https://oauth-openshift.apps.cluster-bmh7t.bmh7t.sandbox3715.opentlc.com/oauth/token/request[token^] from the OpenShift web console. 

////
Click on your username in the top right corner of the OpenShift web console and click *Copy login command*.
