== Elyra Pipeline
We'll run the pipeline from the *summit-workbench* Jupyter notebook we previously opened. Since Elyra is a visual representation of a pipeline it doesn't actually have a runtime built in. When we run the pipeline we are actually submitting the job to OpenShift AI Data Science Pipelines which converts the pipeline run into a Kubelow pipeline run.
...
. Go to the https://rhods-dashboard-redhat-ods-applications.{openshift_cluster_ingress_domain}[OpenShift AI dashboard^] and click on *Data Science Projects* and click on *granite-model-project*. 
...
. Under the *Experiments and runs* section in https://rhods-dashboard-redhat-ods-applications.{openshift_cluster_ingress_domain}[OpenShift AI^] you should see that your pipeline has started running. 
